...
static void _DrawArc(int x0, int y0, int rx, int ry, int Angle0, int Angle1, int xMul, int yMul) {
  float afx[4];
  float afy[4];
	float ri = (float)(rx-(GUI_Context.PenSize+1.5)/2);
	float ro = (float)(rx+(GUI_Context.PenSize+1.5)/2);
  float fAngle0 = (float)(Angle0*3.1415926/180);
  float fAngle1 = (float)(Angle1*3.1415926/180);
  float sin0 = (float)sin(fAngle0); 
  float sin1 = (float)sin(fAngle1); 
  float cos0 = (float)cos(fAngle0); 
  float cos1 = (float)cos(fAngle1); 
  U32   ri2 = (U32)(ri*ri);
  U32   ro2 = (U32)(ro*ro);
	int y, yMax, yMin;
	afy[0] = ri*sin0;
  ...
  int xMinDisp, xMaxDisp, xMin=0,xMax=0;
    for (y=yMax; y>=yMin; y--) {
      ...
      _CalcX(&xMax, y, ro2);
      if ((float)y< afy[1]) {
        xMaxDisp = (int)_CalcInterSectLin((float)y,(float)afy[0], (float)afy[1], (float)afx[0], afx[1]);
			} else {
        xMaxDisp = xMax;			
			}
      if ((float)y > afy[2]) {
        xMinDisp = (int)_CalcInterSectLin((float)y,(float)afy[2], (float)afy[3], (float)afx[2], (float)afx[3]);
			} else {
        xMinDisp = xMin;			
			}
      if (xMul>0)
