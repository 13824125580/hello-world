...
void GUI_SIF__GetFontInfo(const GUI_FONT GUI_UNI_PTR * pFont, GUI_FONTINFO * pfi) {
  ...
  pfi->Baseline = (unsigned char)GUI__Read16(&pData);
  pfi->LHeight  = (unsigned char)GUI__Read16(&pData);
  pfi->CHeight  = (unsigned char)GUI__Read16(&pData);
...
void GUI_SIF_CreateFont(void * pFontData, GUI_FONT * pFont, const GUI_SIF_TYPE * pFontType) {
  ...
  pFont->YSize    = (unsigned char)GUI__Read16(&pSrc);
  pFont->YDist    = (unsigned char)GUI__Read16(&pSrc);
  pFont->Baseline = (unsigned char)GUI__Read16(&pSrc);
  pFont->LHeight  = (unsigned char)GUI__Read16(&pSrc);
  pFont->CHeight  = (unsigned char)GUI__Read16(&pSrc);
