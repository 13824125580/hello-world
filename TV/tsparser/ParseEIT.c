	...
	while (!feof(pfTsFile))
	{
		...
		if (1 == iTemp) //one section over
		{
			if(uiTableId == 0x4e)
			{
				...
				printf("%s line %d, auiRecordGetSection = %d.actual %d.uiTableId = 0x%02x.\n", __func__, __LINE__, auiRecordGetSection[0], aucSectionBuffer[6], uiTableId);
			...
			else if(uiTableId == 0x50)
			{
					unsigned char sec_num = aucSectionBuffer[6];
					unsigned char seg = sec_num >> 3;
					unsigned char index = sec_num & 0x07;
					if(sched[seg] & (1 << index))
					{
						//printf("%s line %d, have one.sec_num = %d. sched[%d] = 0x%02x.\n", __func__, __LINE__, sec_num, seg, sched[seg]);
						continue;
					}
					sched[seg] |= 1 << index;
					iEventCount = ParseEIT_Section(&stTS_EIT, aucSectionBuffer);
