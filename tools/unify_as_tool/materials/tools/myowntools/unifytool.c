...
static const unsigned char data_bin2ascii[16]="0123456789abcdef";
...
int redemo_main(int argc, char **argv)
{
	...
	char rsa_key_text[100];
	int gen_rsa = 0,gen_text = 0;
	int option_index;
	...
	if(gen_text == 1)
	{
		unsigned char outbuf[1100];
		memset(outbuf,0x00,1100);
		outbuf[0]='n';
		outbuf[1]=0x3a;
		outbuf[2]=0x30;
		outbuf[3]='x';
		...
		{
			outbuf[4 + iter*2 + 0] = conv_bin2ascii((publicKey.modulus[iter]>>4)&0xf);
			outbuf[4 + iter*2 + 1] = conv_bin2ascii((publicKey.modulus[iter]>>0)&0xf);
		}
		outbuf[4 + 512 +0] = '\r';
		outbuf[4 + 512 +1] = '\n';
		outbuf[4 + 512 +2] = 'e';
		...
		for(; iter < 256; iter ++)
		{
			outbuf[522 + e*2 + 0] = conv_bin2ascii((publicKey.exponent[iter]>>4)&0xf);
			outbuf[522 + e*2 + 1] = conv_bin2ascii((publicKey.exponent[iter]>>0)&0xf);
			e ++;
		}
		...
		FILE *text_file;
		if ((text_file = fopen (rsa_key_text, "wb+")) == NULL)
		{
			printf("\t\t%s line[%d]:rsa text file failed!\n",
				__func__,__LINE__);
			exit(-1);
		}
		if(fwrite(outbuf,1,524 + 2*e,text_file) != (524 + 2*e))
		{
			printf("\t\t write rsa text file failed!!\n");
			exit(-1);
		}
...
int main(int argc, char **argv)
{
	...
	for(iter = 0; handler_table[iter].op_name;iter ++)
	{
		if(!strcmp(argv[0], handler_table[iter].op_name))
			return handler_table[iter].op_handler(argc,argv);
	}
	printf("unify tools did not have this app!\n");
	return -1;
}
